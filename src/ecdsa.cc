...
int nonce_function_custom(unsigned char* nonce32,
  ...
  v8::Local<v8::Value> argv[] = {
      ...
      algo16 == NULL ? v8::Local<v8::Value>(Nan::Null())
                     : v8::Local<v8::Value>(COPY_BUFFER(algo16, 16)),
      data == NULL ? v8::Local<v8::Value>(Nan::Null())
                   : v8::Local<v8::Value>(COPY_BUFFER(data, 32)),
      Nan::New(counter)};
...
#else
  v8::Local<v8::Value> result =
      noncefn_callback->Call(isolate->GetCurrentContext()->Global(), 5, argv);
#endif

  if (!node::Buffer::HasInstance(result) ||
      node::Buffer::Length(result) != 32) {
    return 0;
  }
...
NAN_METHOD(sign) {
...
#else
    v8::Local<v8::Value> data_value =
        options->Get(Nan::New<v8::String>("data").ToLocalChecked());
#endif
...
#if (NODE_MODULE_VERSION >= NODE_12_0_MODULE_VERSION)
    noncefn_callback = v8::Local<v8::Function>::Cast(
        options
            ->Get(info.GetIsolate()->GetCurrentContext(),
                  Nan::New<v8::String>("noncefn").ToLocalChecked())
            .ToLocalChecked());
#else
    noncefn_callback = v8::Local<v8::Function>::Cast(
        options->Get(Nan::New<v8::String>("noncefn").ToLocalChecked()));
#endif
  ...
  if (secp256k1_ecdsa_sign_recoverable(
          secp256k1ctx, &sig, msg32, private_key, noncefn, data) == 0) {
  ...
  secp256k1_ecdsa_recoverable_signature_serialize_compact(
      secp256k1ctx, &output[0], &recid, &sig);
  ...
  obj->Set(info.GetIsolate()->GetCurrentContext(),
           Nan::New<v8::String>("recovery").ToLocalChecked(),
           Nan::New<v8::Number>(recid));
#else
  obj->Set(Nan::New<v8::String>("signature").ToLocalChecked(),
           COPY_BUFFER(&output[0], 64));
  obj->Set(Nan::New<v8::String>("recovery").ToLocalChecked(),
           Nan::New<v8::Number>(recid));
#endif
...
NAN_METHOD(verify) {
  ...
  if (secp256k1_ecdsa_signature_parse_compact(secp256k1ctx, &sig, sig_input) ==
      0) {
    return Nan::ThrowError(ECDSA_SIGNATURE_PARSE_FAIL);
  }

  secp256k1_pubkey public_key;
  if (secp256k1_ec_pubkey_parse(secp256k1ctx,
                                &public_key,
                                public_key_input,
...
NAN_METHOD(recover) {
  ...
  int recid =
      (int)recid_object->IntegerValue(info.GetIsolate()->GetCurrentContext())
          .ToChecked();
#else
  ...
  UPDATE_COMPRESSED_VALUE(
      flags, info[3], SECP256K1_EC_COMPRESSED, SECP256K1_EC_UNCOMPRESSED);

  secp256k1_ecdsa_recoverable_signature sig;
  if (secp256k1_ecdsa_recoverable_signature_parse_compact(
          secp256k1ctx, &sig, sig_input, recid) == 0) {
    return Nan::ThrowError(ECDSA_SIGNATURE_PARSE_FAIL);
  }
  ...
  secp256k1_ec_pubkey_serialize(
      secp256k1ctx, &output[0], &output_length, &public_key, flags);
