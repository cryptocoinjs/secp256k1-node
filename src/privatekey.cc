...
NAN_METHOD(privateKeyExport) {
  ...
  if (ec_privkey_export_der(
          secp256k1ctx, &output[0], &output_length, private_key, compressed) ==
      0) {
...
NAN_METHOD(privateKeyImport) {
  ...
  if (ec_privkey_import_der(
          secp256k1ctx, &private_key[0], input, input_length) == 0) {
...
NAN_METHOD(privateKeyTweakAdd) {
  ...
  if (secp256k1_ec_privkey_tweak_add(secp256k1ctx, &private_key[0], tweak) ==
      0) {
...
NAN_METHOD(privateKeyTweakMul) {
  ...
  if (secp256k1_ec_privkey_tweak_mul(secp256k1ctx, &private_key[0], tweak) ==
      0) {
