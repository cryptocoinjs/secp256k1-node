...
NAN_METHOD(signatureNormalize) {
  ...
  if (secp256k1_ecdsa_signature_parse_compact(secp256k1ctx, &sigin, input) ==
      0) {
...
NAN_METHOD(signatureExport) {
  ...
  if (secp256k1_ecdsa_signature_serialize_der(
          secp256k1ctx, &output[0], &output_length, &sig) == 0) {
...
NAN_METHOD(signatureImport) {
  ...
  if (secp256k1_ecdsa_signature_parse_der(
          secp256k1ctx, &sig, input, input_length) == 0) {
...
NAN_METHOD(signatureImportLax) {
  ...
  secp256k1_ecdsa_signature sig;
  if (ecdsa_signature_parse_der_lax(secp256k1ctx, &sig, input, input_length) ==
      0) {
    return Nan::ThrowError(ECDSA_SIGNATURE_PARSE_DER_FAIL);
  }
