...
NAN_METHOD(publicKeyCreate) {
  ...
  UPDATE_COMPRESSED_VALUE(
      flags, info[1], SECP256K1_EC_COMPRESSED, SECP256K1_EC_UNCOMPRESSED);
...
NAN_METHOD(publicKeyConvert) {
  ...
  if (secp256k1_ec_pubkey_parse(
          secp256k1ctx, &public_key, input, input_length) == 0) {
...
NAN_METHOD(publicKeyVerify) {
  ...
  secp256k1_pubkey public_key;
  int result =
      secp256k1_ec_pubkey_parse(secp256k1ctx, &public_key, input, input_length);
...
NAN_METHOD(publicKeyTweakAdd) {
  ...
  unsigned int flags = SECP256K1_EC_COMPRESSED;
  UPDATE_COMPRESSED_VALUE(
      flags, info[2], SECP256K1_EC_COMPRESSED, SECP256K1_EC_UNCOMPRESSED);
...
NAN_METHOD(publicKeyCombine) {
  ...
  std::unique_ptr<secp256k1_pubkey[]> public_keys(
      new secp256k1_pubkey[input_buffers->Length()]);
  std::unique_ptr<secp256k1_pubkey*[]> ins(
      new secp256k1_pubkey*[input_buffers->Length()]);
  for (unsigned int i = 0; i < input_buffers->Length(); ++i) {
    ...
    v8::Local<v8::Object> public_key_buffer = v8::Local<v8::Object>::Cast(
        input_buffers->Get(info.GetIsolate()->GetCurrentContext(), i)
            .ToLocalChecked());
#else
    v8::Local<v8::Object> public_key_buffer =
        v8::Local<v8::Object>::Cast(input_buffers->Get(i));
#endif
    CHECK_TYPE_BUFFER(public_key_buffer, EC_PUBLIC_KEY_TYPE_INVALID);
    CHECK_BUFFER_LENGTH2(
        public_key_buffer, 33, 65, EC_PUBLIC_KEY_LENGTH_INVALID);

    const unsigned char* input =
        (unsigned char*)node::Buffer::Data(public_key_buffer);
    size_t input_length = node::Buffer::Length(public_key_buffer);
    if (secp256k1_ec_pubkey_parse(
            secp256k1ctx, &public_keys[i], input, input_length) == 0) {
      return Nan::ThrowError(EC_PUBLIC_KEY_PARSE_FAIL);
    }
  ...
  if (secp256k1_ec_pubkey_combine(
          secp256k1ctx, &public_key, ins.get(), input_buffers->Length()) == 0) {
