...
#define UPDATE_COMPRESSED_VALUE(compressed, value, v_true, v_false)            \
  {                                                                            \
    if (!value->IsUndefined()) {                                               \
      CHECK_TYPE_BOOLEAN(value, COMPRESSED_TYPE_INVALID);                      \
      compressed = value->BooleanValue(info.GetIsolate()->GetCurrentContext()) \
                           .ToChecked()                                        \
                       ? v_true                                                \
                       : v_false;                                              \
    }                                                                          \
...
#define CHECK_NUMBER_IN_INTERVAL(number, x, y, message)                        \
  {                                                                            \
    if (number->IntegerValue(info.GetIsolate()->GetCurrentContext())           \
                .ToChecked() <= x ||                                           \
        number->IntegerValue(info.GetIsolate()->GetCurrentContext())           \
                .ToChecked() >= y) {                                           \
      return Nan::ThrowRangeError(message);                                    \
    }                                                                          \
  }
