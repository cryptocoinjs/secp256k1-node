...
ECPoint.prototype.mulAdd = function (k1, p2, k2) {
  ...
  if (nafPointsP1.wnd !== 1 || nafPointsP2.wnd !== 1) {
    naf = [util.getNAF(k1, nafPointsP1.wnd), util.getNAF(k2, nafPointsP2.wnd)]
    max = Math.max(naf[0].length, naf[1].length)
  } else {
    var comb = [
      p1, /* 1 */
      null, /* 3 */
    ...
    if (p1.y.cmp(p2.y) === 0) {
      comb[1] = p1.add(p2)
      comb[2] = p1.toECJPoint().mixedAdd(p2.neg())
    } else if (p1.y.cmp(p2.y.redNeg()) === 0) {
      comb[1] = p1.toECJPoint().mixedAdd(p2)
      comb[2] = p1.add(p2.neg())
    } else {
      comb[1] = p1.ECJPoint().mixedAdd(p2)
      comb[2] = p1.ECJPoint().mixedAdd(p2.neg())
    }
    ...
    var jsf = util.getJSF(k1, k2)
    max = jsf[0].length
    naf = [new Array(max), new Array(max)]
    for (var j = 0; j < max; j++) {
      var ja = jsf[0][j] | 0
      var jb = jsf[1][j] | 0

      naf[0][j] = index[(ja + 1) * 3 + (jb + 1)]
      naf[1][j] = 0
      wnd[0] = comb
    }
  }
