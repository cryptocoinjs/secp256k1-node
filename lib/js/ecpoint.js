...
ECPoint.fromPublicKey = function (publicKey) {
  ...
  if (publicKey.length === 33 && (first === 0x02 || first === 0x03)) {
    ...
    // create from X
    y = x.redSqr().redIMul(x).redIAdd7().redSqrt()
  ...
  if (publicKey.length === 65 && (first === 0x04 || first === 0x06 || first === 0x07)) {
    ...
    // overflow
    if (x.redIsOverflow() || y.redIsOverflow()) {
      return null
    }
    ...
    // x*x*x + 7 = y*y
    if (!x.redSqr().redIMul(x).redIAdd7().redISub(y.redSqr()).isZero()) {
      return null
    }
...
ECPoint.fromECJPoint = function (p) {
  ...
  var ay = p.y.redMul(zinv2).redIMul(zinv)
...
ECPoint.prototype.add = function (p) {
  ...
  if (!s.isZero()) {
    s = s.redIMul(this.x.redSub(p.x).redInvm())
  }

  var nx = s.redSqr().redISub(this.x).redISub(p.x)
  var ny = s.redIMul(this.x.redSub(nx)).redISub(this.y)
...
ECPoint.prototype.dbl = function () {
  ...
  var s = x2.redAdd(x2).redIAdd(x2).redIMul(yy.redInvm())
